    ...
    if not reader.eof():
        ...
        while not reader.eof():
            ...
            try:
                ...
                # Newer save versions can leave a few extra trailing bytes that don't
                # form a full connector record. Preserve them opaquely instead of crashing.
                reader.data.seek(start_pos)
                data["trailing_bytes"] = list(reader.read_to_end())
                break
            data["other_connectors"].append(entry)
        if "trailing_bytes" not in data and len(data["other_connectors"]) not in [2, 4]:
            print(
