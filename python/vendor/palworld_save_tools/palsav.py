...
import ooz as _ooz  # pyooz: open-source Oodle (Kraken) decompression


def _plm_decompress(data: bytes) -> tuple[bytes, int]:
    import struct as _struct
    uncompressed_len = _struct.unpack_from("<I", data)[0]
    compressed_len = _struct.unpack_from("<I", data, 4)[0]
    save_type = data[11]
    compressed = data[12:]
    if compressed_len != len(compressed):
        raise Exception(f"incorrect compressed length: {compressed_len}")
    out = _ooz.decompress(compressed, uncompressed_len)
    if len(out) != uncompressed_len:
        raise Exception(f"incorrect uncompressed length: {len(out)}")
